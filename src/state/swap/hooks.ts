  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER || currency.symbol === 'BNB' ? 'ETH' : '',
        })
      );
    },
    [dispatch]
...
export function useDerivedSwapInfo(): {
  ...
  const BAD_RECIPIENT_ADDRESSES: string[] = [FACTORY_ADDRESS[chainId!], ROUTER_ADDRESS[chainId!]];
  const {
    independentField,
    typedValue,
